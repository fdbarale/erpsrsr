...
export default function FacturacionModal({
  ...
  const formatoVista = (valor) =>
    '$ ' + Math.round(parseFloat(valor) || 0).toLocaleString('es-AR');

  const baseCubiertaPorPagos = pagos.reduce(
    (acc, p) => acc + p.baseCubierta,
    0
  );
  const saldoRestanteBase = Math.max(
    0,
    Math.round(totalCarrito - baseCubiertaPorPagos)
  );
  ...
  useEffect(() => {
    if (saldoRestanteBase > 0.1) {
      ...
      if (config.recargo < 0) {
        fisicoSugerido =
          saldoRestanteBase / (1 + Math.abs(config.recargo) / 100);
  ...
  const agregarPago = () => {
    ...
    if (config.recargo < 0) {
      ...
      if (baseSaldada > saldoRestanteBase) {
        ...
        fisicoFinalEfectivo = Math.round(
          baseSaldada / (1 + Math.abs(config.recargo) / 100)
        );
      ...
      if (baseSaldada > saldoRestanteBase) {
        baseSaldada = saldoRestanteBase;
        fisicoFinalEfectivo = Math.round(
          baseSaldada * (1 + config.recargo / 100)
        );
      }
  ...
  const buscarCliente = () => {
    ...
    if (!term) return;
    const encontrado = clientesBdSimulada.find(
    ...
    if (encontrado) {
      ...
      else setTipoComprobante('INTERNO');
    } else {
  ...
  const ejecutarColaImpresion = async (nroComprobante) => {
    try {
      ...
      const titular = clienteSeleccionado
        ? clienteSeleccionado.nombre.toUpperCase()
        : 'CONSUMIDOR FINAL';
      const docTitu = clienteSeleccionado ? clienteSeleccionado.doc : '1';
      const condIva = clienteSeleccionado
        ? clienteSeleccionado.tipo.toUpperCase()
        : 'CONSUMIDOR FINAL';

      // 1. DISPARO A LA TIQUEADORA (QZ Tray con comandos puros)
      if (salidaTicket) {
        ...
        comandos.push(
          `1     ${padR('REPUESTOS VARIOS', 17)}  ${padL(
            formatoMoneda(totalCarrito),
            9
          )} ${padL(formatoMoneda(totalCarrito), 9)}\n`
        );
        comandos.push('------------------------------------------\n');

        comandos.push(
          `${padL('SUBTOTAL', 20)} ${padL(formatoMoneda(totalCarrito), 21)}\n`
        );
        if (totalDescuentos > 0)
          comandos.push(
            `DESCUENTO APLICADO ${padL(
        ...
        if (totalRecargos > 0)
          comandos.push(
            `RECARGO FINANCIERO ${padL(
              '+' + formatoMoneda(totalRecargos),
              23
            )}\n`
          );
        comandos.push(
          `${padL('NETO', 20)} ${padL(formatoMoneda(totalNetoAFIP), 21)}\n\n`
        );

        comandos.push('\x1B\x45\x01');
        comandos.push(
          `${padL('TOTAL', 20)} ${padL(formatoMoneda(totalNetoAFIP), 21)}\n`
        );
        ...
        pagos.forEach((p) => {
          comandos.push(
            `${padR(p.metodoNombre, 25)} ${padL(
              formatoMoneda(p.montoFisico),
              16
            )}\n`
          );
        });
        ...
        comandos.push('\n\n\x1B\x61\x01');
        comandos.push(`VTO.: 28-06-2026\n`);
        comandos.push(`CAE: 86250273394935\n\n`);
      ...
      // 2. DISPARO A4 CON VISTA PREVIA EN NAVEGADOR
      if (salidaA4) {
        const htmlA4 = `
          <div style="font-family: Arial, sans-serif; color: #000; padding: 20px; font-size:14px; max-width: 800px; margin: auto;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
              ...
              <tbody>
                <tr>
                  ...
                  <td style="padding: 15px 10px; text-align: right; border-bottom: 1px solid #eee; font-family: monospace; font-size: 16px;">${formatoVista(
                    totalCarrito
                  )}</td>
                </tr>
              </tbody>
            ...
            <div style="display: flex; justify-content: space-between;">
              <div style="width: 45%; border: 1px solid #ccc; padding: 15px; border-radius: 8px;">
                ...
                <ul style="list-style: none; padding: 0; margin: 0; font-family: monospace;">
                  ${pagos
                    .map(
                      (p) =>
                        `<li style="display:flex; justify-content:space-between; margin-bottom:5px;"><span>${
                          p.metodoNombre
                        }</span> <strong>${formatoVista(
                          p.montoFisico
                        )}</strong></li>`
                    )
                    .join('')}
              ...
              <div style="width: 45%; text-align: right; line-height: 2;">
                Subtotal Original: <span style="font-family: monospace;">${formatoVista(
                  totalCarrito
                )}</span><br>
                ${
                  totalDescuentos > 0
                    ? `Descuentos Totales (-): <span style="font-family: monospace; color: green;">${formatoVista(
                        totalDescuentos
                      )}</span><br>`
                    : ''
                }
                ${
                  totalRecargos > 0
                    ? `Recargos Totales (+): <span style="font-family: monospace; color: red;">${formatoVista(
                        totalRecargos
                      )}</span><br>`
                    : ''
                }
                <div style="border-top: 2px solid #000; margin-top: 10px; padding-top: 10px;">
                  <h2 style="margin: 0; color: #000; font-size: 22px;">TOTAL LIQUIDADO: <span style="font-family: monospace;">${formatoVista(
                    totalNetoAFIP
                  )}</span></h2>
                </div>
              </div>
        ...
        // Magia para abrir en pestaña nueva y disparar la vista previa del navegador
      ...
      alert('Error Hardware: ' + err.message);
    }
  };
  ...
  const finalizarVenta = async () => {
    ...
    const nroComprobante =
      '0001-' + Math.floor(10000000 + Math.random() * 90000000);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h5 className="mb-0 fw-bold tracking-wide">📦 Cierre de Caja AFIP</h5>
          <button
        ...
        <div className="card-body p-0 d-flex flex-row overflow-hidden bg-light h-100">
          <div className="col-5 p-3 d-flex flex-column border-end bg-white h-100">
            ...
            <div className="d-flex gap-2 mb-3">
              <select
                className="form-select form-select-sm fw-bold bg-light shadow-sm"
                style={{ width: '45%' }}
                value={metodoActual}
              ...
              <div
                ...
                style={{ width: '55%' }}
              >
                <span className="input-group-text fw-bold text-success bg-white border-end-0">
                  $
                </span>
                <input
                  type="number"
                  ...
                  {pagos.length === 0 ? (
                    ...
                    pagos.map((p) => (
                      <tr key={p.id} className="border-bottom">
                        <td
                          className="fw-semibold text-dark ps-2"
                          style={{ fontSize: '0.8rem' }}
                        >
                          {p.metodoNombre}
                        </td>
                        <td className="text-end font-monospace text-dark fw-bold small">
                          {formatoVista(p.montoFisico)}
                        </td>
                        <td className="text-end font-monospace text-secondary small p-2 pe-2">
                          {formatoVista(p.baseCubierta)}
                        </td>
                      </tr>
                    ))
                  )}
              ...
              <div className="card-body p-3">
                ...
                <div className="d-flex justify-content-between align-items-center mt-2 border-top pt-2">
                  ...
                  <span className="fs-4 fw-bolder font-monospace text-dark">
                    {formatoVista(totalNetoAFIP)}
                  </span>
                </div>

                {saldoRestanteBase > 1 ? (
                  <div
                    className="text-center p-2 rounded-2 mt-2 fw-bold text-danger small"
                    style={{ border: '1px solid #dc3545' }}
                  >
                    RESTRICCIÓN DE SALDO: FALTA{' '}
                    {formatoVista(saldoRestanteBase)}
                  </div>
                ) : (
                  <div
                    className="text-center p-2 rounded-2 mt-2 fw-bold text-white small"
          ...
          <div className="col-5 p-3 d-flex flex-column border-end bg-light h-100 overflow-auto">
            ...
            <div className="input-group input-group-sm mb-3 shadow-sm">
              <input
                ...
                placeholder="DNI, CUIT, Razón Social..."
            ...
            {clienteSeleccionado ? (
              <div className="card border-success shadow-sm mb-3">
                <div className="card-body p-2">
                  ...
                  >
                    CUIT/DNI: {clienteSeleccionado.doc}
                  </span>
                </div>
              </div>
            ) : (
            ...
            <div className="card border-0 shadow-sm mt-auto">
              <div className="card-body p-2">
                <label
                  className="fw-bold text-secondary small mb-2 d-block text-uppercase"
                  style={{ fontSize: '0.75rem' }}
                >
                  Destino Fiscal
                </label>
                <div
                  className="btn-group-vertical w-100 shadow-sm"
                  role="group"
                >
                  <input
                    type="radio"
                  ...
                  />
                  <label
                    className="btn btn-outline-secondary fw-bold text-start py-2"
                    htmlFor="btnradio2"
                  >
                    📄 Comprobante Interno
                  </label>
                  ...
                  />
                  <label
                    className="btn btn-outline-primary fw-bold text-start py-2"
                    htmlFor="btnradio1"
                  >
                    🧾 Factura Oficial AFIP (A/B)
                  </label>
                </div>
              </div>
            </div>
          ...
          <div className="col-2 p-3 d-flex flex-column bg-white h-100">
            <h6
              className="fw-bold mb-3 text-secondary text-uppercase border-bottom pb-1 text-center"
              style={{ fontSize: '0.85rem' }}
            >
              Impresión
            </h6>
            <div className="d-flex flex-column gap-2 h-100">
              <div
                className={`form-check p-2 border rounded-3 d-flex flex-column align-items-center justify-content-center text-center flex-grow-1 ${
                  salidaTicket
        ...
        <div className="card-footer bg-white border-top p-3 d-flex justify-content-between align-items-center">
          ...
          <button
            ...
            style={{ backgroundColor: colorBordo }}
            disabled={saldoRestanteBase > 1}
            onClick={finalizarVenta}
          >
            CONFIRMAR Y EMITIR (Enter)
          </button>
        </div>
      </div>
    </div>
