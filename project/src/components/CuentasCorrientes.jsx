...
export default function CuentasCorrientes({ onLevantarComprobante, volverAlMenu }) {
  ...
  const ejecutarCobroCombinado = async () => {
    ...
    try {
      ...
      // 2. Pegamos en la base Interna
      const { error: errorInterna } = await dbInterna
        .from('clientes')
        .update({ 
      ...
      <nav className="navbar navbar-dark shadow-sm px-3" style={{ backgroundColor: colorBordo, borderBottom: `4px solid ${colorGris}` }}>
        <div className="container-fluid p-0">
          <div className="d-flex align-items-center">
            ...
            <span className="navbar-brand fw-bold m-0 tracking-wide">Cuentas Corrientes y Billetera</span>
          </div>
      ...
      <div className="container-fluid px-4 mt-3 flex-grow-1 d-flex flex-column mb-5 pb-5">
        ...
        <div className="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3 bg-white p-3 rounded shadow-sm">
          ...
          <div className="d-flex gap-3 align-items-center">
            ...
            <div className="form-check form-switch bg-light border rounded px-4 py-2 mb-0 shadow-sm">
              <input className="form-check-input ms-0" type="checkbox" id="verInterno" checked={verInterno} onChange={() => setVerInterno(!verInterno)} style={{cursor:'pointer'}} />
              <label className="form-check-label small fw-bold text-secondary ms-2" htmlFor="verInterno" style={{cursor:'pointer'}}>Ver Operaciones Ocultas</label>
            </div>
          </div>
        </div>
                ...
                <div className="flex-grow-1 border rounded bg-white overflow-auto shadow-sm mb-3" style={{maxHeight:'30vh', minHeight:'20vh'}}>
                  <table className="table table-hover mb-0 align-middle">
                    ...
                    <tbody>
                      ...
                      {clienteSeleccionado.historial.filter(h => verInterno ? true : h.fiscal).length === 0 && (
                ...
                <div className="card border shadow-sm p-4 bg-white mt-auto rounded-3">
                  ...
                  <div className="row g-3 mb-3">
                    ...
                    <div className="col-md-3">
                      ...
                      <input type="number" className="form-control font-monospace text-dark fw-bold border-success" value={pagoBilletera} onChange={e => setPagoBilletera(e.target.value)} placeholder={`Máx: ${clienteSeleccionado.saldo_billetera_negro}`} />
                    </div>
                  </div>
                  ...
                  <div className="border-top pt-3 d-flex justify-content-between align-items-center">
                    ...
                    <div className="d-flex gap-3">
                      <button className="btn btn-outline-dark fw-bold px-4 py-2 shadow-sm" onClick={procesarLevantarAFacturar}>
                        🚀 LEVANTAR PARA FACTURAR
                      </button>
                      <button className="btn text-white fw-bold px-5 py-2 shadow" style={{backgroundColor: colorBordo}} onClick={ejecutarCobroCombinado}>
                        LIQUIDAR COBRO COMBINADO
                      </button>
                    </div>
                  </div>
