...
export default function Mostrador({
  ...
  useEffect(() => {
    ...
    const filtrados = baseDatos.filter((item) => {
      ...
      const textoCompleto = `${item.cod} ${item.desc} ${
        item.codigo_aux || ''
      } ${item.distribuidor}`.toLowerCase();
  ...
  const actualizarCantidadPedido = (cod, incremento) => {
    const modificarItem = (lista) =>
      lista.map((item) => {
        if (item.cod === cod)
          return {
            ...item,
            cant_pendiente: Math.max(
              0,
              (item.cant_pendiente || 0) + incremento
            ),
          };
        return item;
      });
    setBaseDatos((prev) => modificarItem(prev));
    if (faseBusqueda === 'EQUIVALENCIAS')
      setListaEquivalencias((prev) => modificarItem(prev));
  };
  ...
  const manejarTecladoBuscador = (e) => {
    ...
    if (e.key === 'Insert' || e.key === 'Delete') {
      ...
      if (
        ...
        indiceSubFoco >= 0 &&
        listaEquivalencias[indiceSubFoco]
      ) {
        e.preventDefault();
        actualizarCantidadPedido(
          listaEquivalencias[indiceSubFoco].cod,
          incremento
        );
      }
      return;
    ...
    if (faseBusqueda === 'BUSQUEDA') {
      ...
      if (e.key === 'ArrowDown') {
        ...
        if (indiceFoco >= 0 && resultados[indiceFoco]) {
          ...
          const equivalentes = baseDatos.filter(
            (i) => i.codigo_aux === itemActual.codigo_aux
          );
        ...
        if (indiceFoco >= 0 && resultados[indiceFoco])
          ...
          procesarSeleccionArticulo(resultados[0]);
      }
    } else if (faseBusqueda === 'EQUIVALENCIAS') {
      if (listaEquivalencias.length === 0) return;
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setIndiceSubFoco((prev) =>
          prev < listaEquivalencias.length - 1 ? prev + 1 : prev
        );
      } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        ...
        e.preventDefault();
        if (indiceSubFoco >= 0 && listaEquivalencias[indiceSubFoco])
          iniciarSeleccionDePrecio(listaEquivalencias[indiceSubFoco]);
      }
    } else if (faseBusqueda === 'PRECIOS') {
      if (opcionesPrecio.length === 0) return;
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setIndicePrecioFoco((prev) =>
          prev < opcionesPrecio.length - 1 ? prev + 1 : prev
        );
      } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        ...
        e.preventDefault();
        if (indicePrecioFoco >= 0 && opcionesPrecio[indicePrecioFoco])
          confirmarAgregarAlCarrito(
            itemPendiente,
            opcionesPrecio[indicePrecioFoco].precio
          );
      }
    }
  };

  const procesarSeleccionArticulo = (repuesto) =>
    confirmarAgregarAlCarrito(repuesto, repuesto.precio);

  const iniciarSeleccionDePrecio = (repuestoElegido) => {
    setItemPendiente(repuestoElegido);
    const primos = baseDatos.filter(
      (i) => i.codigo_aux === repuestoElegido.codigo_aux
    );
    const precioMaximo = Math.max(...primos.map((p) => p.precio));
    const limitePermitido =
      precioMaximo * (1 + CONFIG_MARGEN_MAXIMO_PORCENTAJE / 100);
  ...
  const confirmarAgregarAlCarrito = (repuesto, precioFinal) => {
    const nuevoCarrito = [
      ...carrito,
      { ...repuesto, precio: precioFinal, cantidad: 1, esManual: false },
    ];
    ...
    return (
      <div className="d-inline-flex ms-2 gap-1 align-items-center flex-nowrap">
        {item.cant_en_camino > 0 && (
          <span
            className="badge text-warning border border-warning px-1"
        ...
        {item.cant_pendiente > 0 && (
          <span
            className="badge text-info border border-info px-1"
            style={{ fontSize: '0.75rem' }}
          >
            🛒 {item.cant_pendiente}
          </span>
  ...
  const renderInsigniaStock = (stock) => {
    return stock > 0 ? (
      <span
        className="badge bg-success bg-opacity-10 text-success border border-success mx-2"
        style={{ fontSize: '0.75rem' }}
      >
        Stock: {stock}
      </span>
      ...
      <span
        className="badge bg-danger bg-opacity-10 text-danger border border-danger mx-2"
        style={{ fontSize: '0.75rem' }}
      >
        Stock: {stock}
      </span>
  ...
  const totalVenta = carrito.reduce(
    (acum, item) =>
      acum + (Number(item.precio) || 0) * (Number(item.cantidad) || 0),
    0
  );
  const totalArticulos = carrito.reduce(
    (acum, item) => acum + (Number(item.cantidad) || 0),
    0
  );
  const colorFiltro =
    modoFiltro === 'LOCAL'
      ...
      >
        <div className="container-fluid p-0">
          <div className="d-flex align-items-center">
            ...
            <span className="navbar-brand fw-bold m-0 tracking-wide">
              RSR - Mostrador Ágil
            </span>
          </div>
        ...
        <div className="row h-100">
          <div className="col-lg-9 col-xl-10">
            <div className="d-flex mb-3 position-relative">
              <div
                className="w-100 position-relative d-flex align-items-center bg-white border shadow-sm rounded-pill px-2"
                style={{ borderColor: '#ced4da', zIndex: 1060 }}
              >
                <span
                  className={`badge ${colorFiltro} ms-2 rounded-pill`}
                  style={{ cursor: 'pointer', padding: '0.4em 0.8em' }}
                  onClick={() => setModoFiltro('LOCAL')}
                >
                  [F3] {modoFiltro}
                </span>
                <input
                ...
                {resultados.length > 0 && (
                  <div
                    className="position-absolute w-100 shadow-lg bg-white rounded-3"
                    style={{
                      top: '110%',
                      left: '0',
                      zIndex: 1050,
                  ...
                  >
                        ...
                        {resultados.map((item, idx) => {
                          ...
                          return (
                            ...
                            >
                              <div className="d-flex align-items-center w-75">
                                <strong
                                  className={`font-monospace ${
                                    esActivo ? 'text-dark' : 'text-primary'
                                  }`}
                                >
                                  {item.cod}
                                </strong>
                                ...
                                <div className="ms-auto d-flex align-items-center flex-nowrap">
                                  {item.codigo_aux ? (
                                    ...
                                    <span
                                      className="badge bg-secondary bg-opacity-10 text-secondary border border-secondary mx-2"
                                      style={{ fontSize: '0.75rem' }}
                                    >
                                      Sin internalizar
                                    </span>
                                  ...
                                  {tienePrimos && (
                                    <span
                                      className="badge bg-info bg-opacity-10 text-info border border-info ms-2"
                                      style={{ fontSize: '0.75rem' }}
                                    >
                                      🔗 Equivalencias (➔)
                                    </span>
                              ...
                              <div className="fw-bold font-monospace w-25 text-end pe-2 text-dark">
                    ...
                    {faseBusqueda === 'EQUIVALENCIAS' && (
                      <div className="bg-light">
                        <div
                          className="p-2 border-bottom text-muted fw-bold d-flex justify-content-between align-items-center"
                          style={{ backgroundColor: '#e9ecef' }}
                        >
                          <span className="small">⬅️ (Izq) Volver</span>
                          <span className="text-dark small fw-bold">
                            Equivalencias Disponibles (Ins/Supr para Pedido
                            directo)
                          </span>
                          ...
                          {listaEquivalencias.map((item, idx) => {
                            ...
                            return (
                              <li
                                key={idx}
                                className="list-group-item d-flex justify-content-between align-items-center py-2 border-bottom"
                                onClick={() => iniciarSeleccionDePrecio(item)}
                                style={{
                                  cursor: 'pointer',
                                  backgroundColor: esActivo
                                    ? '#d0e7ff'
                              ...
                              >
                                ...
                                <div className="fw-bold text-dark font-monospace text-end w-25 pe-2">
                                  ...
                                  {esActivo && (
                                    <span
                                      className="d-block text-muted font-sans"
                                      style={{ fontSize: '0.65rem' }}
                                    >
                                      (Enter) Elegir para Cobrar
                                    </span>
                                  )}
                                </div>
                              </li>
                    ...
                    {faseBusqueda === 'PRECIOS' && (
                      <div className="bg-light">
                        <div
                          className="p-2 border-bottom fw-bold text-center d-flex flex-column align-items-center"
                        ...
                        >
                          ...
                          <span
                            ...
                            style={{ fontSize: '0.75rem' }}
                          >
                            Seleccione el precio de lista que desea aplicarle a
                            este despacho
                          </span>
                        </div>
                        <ul
                            ...
                            return (
                              ...
                              >
                                <div className="d-flex align-items-center w-75">
                                  ...
                                  >
                                    Aplicar precio de lista de:
                                  </span>
                                  <strong className="font-monospace text-dark ms-2">
                                    {opcion.cod}
                                  </strong>
              ...
              <div className="card-body p-0">
                <table className="table table-borderless table-hover mb-0 align-middle w-100">
                  ...
                  >
                    ...
                    >
                      <th style={{ width: '15%' }} className="ps-3 py-2">
                        Código
                      </th>
                      <th style={{ width: '45%' }} className="py-2">
                        Descripción
                      </th>
                      <th style={{ width: '10%' }} className="text-center py-2">
                        Cant.
                      </th>
                      <th style={{ width: '12%' }} className="text-end py-2">
                        Unitario
                      </th>
                      <th
                        style={{ width: '15%' }}
                        className="text-end pe-3 py-2"
                      >
                        Subtotal
                      </th>
                      <th style={{ width: '3%' }}></th>
                    ...
                    {carrito.length === 0 ? (
                      <tr>
                        <td colSpan="6" className="text-center text-muted py-4">
                          <span className="d-block fs-2 mb-1 opacity-25">
                            🛒
                          </span>
                      ...
                      carrito.map((item, index) => (
                        <tr key={index} className="border-bottom">
                          <td className="fw-bold font-monospace text-primary ps-3">
                            {item.cod}
                          </td>
                          <td>
                            {item.esManual ? (
                              <input
                                type="text"
                                className="form-control form-control-sm border-0 bg-light fw-bold w-100"
                                value={item.desc}
                                onChange={(e) =>
                                  cambiarDatoManual(
                                    index,
                          ...
                          <td className="text-center">
                            <input
                              type="number"
                              className="form-control form-control-sm text-center font-monospace fw-bold bg-light mx-auto"
                              style={{ maxWidth: '70px' }}
                              value={item.cantidad}
                              onChange={(e) =>
                              ...
                              onKeyDown={(e) =>
                                manejarTecladoCantidad(e, index, item.esManual)
                              }
                              ref={(el) => (cantidadesRef.current[index] = el)}
                            />
                          </td>
                          <td className="text-end">
                            {item.esManual ? (
                              <div className="input-group input-group-sm justify-content-end">
                                <span className="input-group-text bg-transparent border-0 text-success fw-bold pe-1 px-1">
                                  $
                                </span>
                                <input
                                  type="number"
                                  className="form-control form-control-sm text-end font-monospace fw-bold text-success bg-light"
                                  style={{ maxWidth: '80px' }}
                                  value={item.precio || ''}
                                  onChange={(e) =>
                                    cambiarDatoManual(
                                  ...
                                  onKeyDown={manejarTecladoPrecio}
                                  ref={(el) => (preciosRef.current[index] = el)}
                                />
                              </div>
                            ) : (
                          ...
                          <td className="text-end pe-2">
                            <button
                              className="btn btn-sm text-danger opacity-50 p-1"
          ...
          <div className="col-lg-3 col-xl-2">
            <div className="card border shadow-sm rounded-3 bg-white mb-2">
              <div className="card-body p-3 text-center">
                <h6 className="text-uppercase text-secondary fw-bold mb-1 small">
                  Total Carrito
                </h6>
                ...
                >
                  <span>
                    Artículos:{' '}
                    <strong className="text-dark">{totalArticulos}</strong>
                  </span>
                </div>
                <div className="d-grid gap-2">
                  <button
                    className="btn btn-sm fw-bold py-2 text-white shadow-sm"
                    style={{ backgroundColor: colorBordo, borderRadius: '6px' }}
                    tabIndex="-1"
                    onClick={() => {
                      if (carrito.length > 0) setMostrarFacturacion(true);
                    }}
                  ...
                  <button
                    className="btn btn-sm btn-light border-secondary border-opacity-25 fw-bold py-2 text-dark shadow-sm"
                  ...
                  <button
                    className="btn btn-sm btn-white border fw-bold py-2 text-secondary shadow-sm"
                    style={{ borderRadius: '6px' }}
                    tabIndex="-1"
                    disabled
                  >
                  ...
                  <button
                    className="btn btn-sm btn-link text-danger text-decoration-none fw-semibold p-0 mt-1 small"
                    tabIndex="-1"
